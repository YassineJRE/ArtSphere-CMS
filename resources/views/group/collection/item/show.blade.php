...
@section('page-title')
    <a href="{{ route('app.groups.collections.show', [
            'group' => $group->id,
...
@section('breadcrumbs')
    @include('components.breadcrumbs', ['breadcrumbs' => array_merge(
        ...
        [
            ...
            __('my-collection.views.show.breadcrumbs.my-collections') => route('app.groups.collections.index', [
                'group' => $group->id,
                'search' => request()->search,
                'discover' => request()->discover,
            ]),
            $collection->title => route('app.groups.collections.show', [
                'group' => $group->id,
                'collection' => $collection->id,
                'search' => request()->search,
                'discover' => request()->discover,
...
@section('navigation-right-side')
    @if ($item->isExhibit() || $item->isCollection())
        <a href="@if ($item->model->belongsToProfile())
                {{ route('app.profiles.show', ['profile' => $item->model->owner_id, 'search' => request()->search, 'discover' => request()->discover]) }}
            @elseif ($item->model->belongsToGroup())
                {{ route('app.groups.show', ['group' => $item->model->owner_id, 'search' => request()->search, 'discover' => request()->discover]) }}
            @endif
        ">{{ $item->model->owner->getName() }}</a>
    @elseif ($item->isArtwork())
        <a href="@if ($item->model->exhibit->belongsToProfile())
                {{ route('app.profiles.show', ['profile' => $item->model->exhibit->owner_id, 'search' => request()->search, 'discover' => request()->discover]) }}
            @elseif ($item->model->exhibit->belongsToGroup())
                {{ route('app.groups.show', ['group' => $item->model->exhibit->owner_id, 'search' => request()->search, 'discover' => request()->discover]) }}
            @endif
