@isset($myProfile)
        ...
        <div class="woocommerce col-md-12">
            <form
                @if ($myProfile->isProfile())
                    action="{{ route('my-profile.my-exhibits.store',[
                        'my_profile' => $myProfile->id
                    ]) }}"
                @elseif ($myProfile->isGroup())
                    action="{{ route('my-group.my-exhibits.store',[
            ...
            >
                ...
                <div class="form-row col-md-5">
                    <label for="name">{{ __('my-exhibit.views.create.form.p.label.exhibit-name') }} <span class="required">*</span></label>
                    <input
                        type="text"
                ...
                <div class="form-row col-md-4">
                    <label for="type">{{ __('my-exhibit.views.create.form.p.label.type') }} <span class="required">*</span></label>
                    <select
                    ...
                    >
                        ...
                        @foreach ($exhibitTypes as $exhibitType)
                            <option
                                value="{{ $exhibitType }}"
                                @if (@old('type') && @old('type') == $exhibitType)
                                    selected
                                @endif
                ...
                <div class="form-row col-md-12">
                    <label for="description">{{ __('my-exhibit.views.create.form.p.label.description') }}</label>
                    <textarea
                ...
                <div class="form-row col-md-12">
                    <span style="color: #ff6600;">{{ __('my-exhibit.views.create.form.p.label.description-text') }}</span>
                </div>
                ...
                <div class="form-row col-md-12">
                    ...
                    @include('my-profile.my-exhibit.gallery-select', [
                        ...
                        'preselected' => ['value'=>old('verifier_id'),'text'=>old('gallery_name')]
                    ])
                    ...
                    <input type="hidden" id="gallery_id" name="verifier_id" value="{{ old('verifier_id') }}">
                    <input type="hidden" id="gallery_name" name="gallery_name" value="{{ old('gallery_name') }}">
                ...
                <!-- Just for the inviteGallery function atm, could be useful for something else in future though -->
				<script src="{{ asset('js/scripts/autocomplete.js') }}"></script>	

                
                <div class="form-row col-md-12">
                    <label for="location">{{ __('my-exhibit.views.create.form.p.label.location') }}</label>
						<input
							type="text"
                ...
                <div class="form-row col-md-6">
                    <label for="upcoming_date">{{ __('my-exhibit.views.create.form.p.label.upcoming-date') }}</label>
                    <input
                        type="text"
                        class="input-text input-date form-control @if ($errors->has('upcoming_date')) invalid @endif"
                        name="_upcoming_date"
                        id="upcoming_date"
                        value="{{ old('upcoming_date') ? \Carbon\Carbon::parse(old('upcoming_date'))->format('m/d/Y') : '' }}"
                        placeholder=""
                    >
                    <input
                        type="hidden"
                        name="upcoming_date"
                        value="{{ old('upcoming_date') ? \Carbon\Carbon::parse(old('upcoming_date'))->format('Y-m-d') : '' }}"
                    >
                ...
                <div class="form-row col-md-6">
                    <label for="open_at">{{ __('my-exhibit.views.create.form.p.label.open-at') }}</label>
                    <input
                        type="text"
                        class="input-text input-datetime form-control @if ($errors->has('open_at')) invalid @endif"
                        name="_open_at"
                        id="open_at"
                        value="{{ old('open_at') ? \Carbon\Carbon::parse(old('open_at'))->format('m/d/Y h:m A') : '' }}"
                    >
                    <input
                        type="hidden"
                        name="open_at"
                        value="{{ old('open_at') ? \Carbon\Carbon::parse(old('open_at'))->format('Y-m-d h:m:s') : '' }}"
                    >
                ...
                <div class="form-row col-md-12">
                    <label for="special_thanks">{{ __('my-exhibit.views.create.form.p.label.thanks') }}</label>
                    <textarea
                ...
                <div class="form-row col-md-12 margin-top-15">
                    <p>{{ __('my-exhibit.views.create.form.p.label.additional-information-content') }}</p>
                </div>
                <div class="form-row col-md-6">
                    <input
                        type="text"
                        ...
                        placeholder="{{ __('my-exhibit.views.create.form.p.label.additional-information-title') }}"
                    >
                ...
                @if ($myProfile->isArtist())
                    <div class="form-row col-md-6">
                        <label for="status">{{ __('my-exhibit.views.create.form.p.label.status') }}</label>
                        <select
                ...
                @endif
                @if ($myProfile->canTransferExhibit())
                    <div class="form-row col-md-12 margin-top-10">
                        {{ __('my-exhibit.views.create.form.p.transfer-text') }}
                    </div>
                @endif
                <div class="form-row col-md-12 my-profile @if ($myProfile->canTransferExhibit())with-separator @endif">
                    <div class="margin-top-10">
                        ...
                        <input
                            ...
                            class="button"
                            name="save"
                            value="{{ __('my-exhibit.views.create.form.p.button.save') }}"
                        >
                        @if ($myProfile->canTransferExhibit())
                            <input
                                type="submit"
                                ...
                                name="transfer"
                                value="{{ __('my-exhibit.views.create.form.p.button.transfer') }}"
                            >
                        @endif
                    </div>
                </div>
    ...
    @push('scripts')
		{{-- <script src="{{ asset('js/scripts/draganddrop.js') }}"></script> --}}
