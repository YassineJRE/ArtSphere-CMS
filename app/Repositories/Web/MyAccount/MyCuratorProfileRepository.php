...
class MyCuratorProfileRepository extends BaseRepository
{
    /**
    ...
    public function create(array $data): UserProfile
    {
        try {
            $profile = DB::transaction(function () use ($data) {
                $user = Auth::user();
                $data['type'] = ProfileType::CURATOR;

                if ($user && !$user->hasProfileCurator()) {
                ...
                return $this->update($user->profileCurator(), $data);                 
            });
